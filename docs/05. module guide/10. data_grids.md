...
private function loadDataGrid($published)
{
	$dg = new BackendDataGridDatabase(BackendMiniBlogModel::QUERY_DATAGRID_BROWSE,
					array($published, BackendLanguage::getWorkingLanguage()));
```
...
```
	$dg->setHeaderLabels(array('user_id' => ucfirst(BackendLanguage::lbl('Author'))));
```
...
...
	...
	$dg->addColumn('edit', null, BackendLanguage::lbl('Edit'), BackendModel::createUrlForAction('edit') .
				'&amp;id=[id]', BackendLanguage::lbl('Edit'));

	$dg->addColumn('delete', null, BackendLanguage::lbl('Delete'), 		
		BackendModel::createUrlForAction('delete') . '&amp;id=[id]', BackendLanguage::lbl('Delete'));
```

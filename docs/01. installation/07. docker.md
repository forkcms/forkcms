...
### What's inside the Fork CMS Docker dev environment?
A `docker-compose`-powered stack to get your Fork CMS PHP project up and running in no time, without the hassle of 
installing and configuring PHP yourself locally, like in a Vagrant or in MAMP/WAMP. 
...
**Features:**
* Easily switch between PHP versions: 7.1, 7.2, ... by editting the Dockerfile to fit your needs.
* Easy to customize, e.g. add PHP extensions, with a simple edit to the `Dockerfile`.
* Clean and well structured Dockerfile
* All containers extend from an official base image (trusted base images).
* Everything is visible and editable to fit your needs of the project.
* Fast image builds.
* Any teammember can quickly start up your project because the Dockerfile and docker-compose file is added to your version control.
...
php extensions, custom php.ini settings, Xdebug, install the composer dependencies, ... This Dockerfile will be used
to build a Docker "image" which is used to start up the container. 
* Our `docker-compose.yml` file, found in the root, describes how our services work together. We only have 2 services: the app and the
db service. In the `docker-compose.yml` file, we specify that the app-service is linked and can communicate with the
...
```
You should see something similar appear: 2 containers are up and running. One container for the webserver/php and one
...
- Finish the installation process. That it, enjoy your fresh local Fork CMS installation powered by Docker.


## Configuring your local develop environment in PHPStorm
### XDebug
XDebug is one of the most popular tools to debug your PHP application. It's easy to set it up in PHPStorm. 
...
- You can edit the `.env` file to fill in some host-specific information and configure XDebug. 
- Restart/rebuild your Docker containers, because you modified the env variables:
```
docker-compose build
docker-compose up -d
```

- In PHPStorm's preferences, go to the PHP > Debug section. Make sure Xdebug accepts connections and listens on port 9000 (should be default configuration).
![Configuring XDebug in PHPStorm](assets/phpstorm-xdebug1.png)
...
will detect the incoming connection and set up the server configuration automatically.
![Configuring a server in PHPStorm](assets/phpstorm-xdebug2.png) 

- Place a breakpoint in your code, start listening for incoming debug connections by clicking the green telephone icon
![Start listening for incoming debug connections](assets/phpstorm-xdebug3.png). Now visit http://localhost or the page you
...
root@b50c9fefe23d:/var/www/html# bin/console forkcms:cache:clear    # Run your command
```
...
| Stop a single container                                  | `docker-compose {container-name} stop`       |
| Delete all existing containers                           | `docker-compose down` *                       |
| Build the docker containers (after modifying Dockerfile) | `docker-compose build`                       |
| View the Log files                                       | `docker-compose logs -f -t {container-name}` |

\* *You can safely delete all containers, because your PHP files simply get mounted into the Docker container. 
You won't lose your database either, as this gets mounted from the `/var/docker/db/data` directory when you start containers.*


## FAQ
### I want to run a Symfony command inside a running container
...
again and it will restart the container that changed, or just run `docker-compose restart`.

### I want to add more software to my project (Docker images)
Imagine that your project also needs a Redis key-value store, or a MongoDB, or Elasticsearch, or RabbitMQ, ...
...
which speeds up performance by 2.7x.
...
Interested in deploying Docker containers? Check out [Docker Swarm](https://github.com/docker/swarm), [Dokku](http://dokku.viewdocs.io/dokku/), [Kubernetes](https://kubernetes.io/), or

...
function convert($filedata)
{
    ...
    $filedata = preg_replace_sprintf('/\|usersetting:(.*?)}}/', '|usersetting(%s}})', $filedata);
    $filedata = preg_replace_sprintf('/\|geturlforblock:(.*?)}}/', '|geturlforblock(%s}})', $filedata);
    ...
    // caching
    $filedata = preg_replace_sprintf('/{\/cache:(.*?)}/i', '{# endcache #}', $filedata);
    $filedata = preg_replace_sprintf('/{cache:(%s)}/i', '{# cache %s #}', $filedata);
...
if ($force === true) {
    ...
    if (!isFile(str_replace('.tpl', EXTENSION, $input)))
    {
        write($input, convert(getFile($input)));
    }

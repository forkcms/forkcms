	...
	public static function getNavigationHTML($type = 'page', $parentId = 0, $depth = null, $excludeIds = array(), $depthCounter = 1)
	{
		...
		while(true)
		{
			...
			foreach($navigation[$type][$parentId] as $id => $page)
			{
				...
				if(array_search($id, self::$excludedPageIds) !== false)
				{
					unset($navigation[$type][$parentId][$id]);
					continue;
				}
				...
				if(isset($navigation[$type][$page['page_id']]) && $page['page_id'] != 1 && ($depth == null || $depthCounter + 1 <= $depth)) $navigation[$type][$parentId][$id]['children'] = self::getNavigationHTML($type, $page['page_id'], $depth, $excludeIds, $depthCounter + 1);				else $navigation[$type][$parentId][$id]['children'] = false;

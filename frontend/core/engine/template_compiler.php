...
class FrontendTemplateCompiler extends SpoonTemplateCompiler
{
    ...
    {
        // regex pattern
        // no unified restriction can be done on the allowed characters, that
        // differs from one OS to another
            ...
            foreach ($matches as $match) {
                ...
                $replace = '<?php $includes = array();
                           ...
                           '<?php
                                ...
                                ob_start();
                            ?>' . $this->variables['THEME_PATH'] . '/' . $match[1] .
                           '<?php
                                $includes[] = str_replace(\'//\', \'/\', eval(\'return \\\'\' . str_replace(\'\\\'\', \'\\\\\\\'\', ob_get_clean()) .\'\\\';\'));
                                ob_start();
                           ?>' . $this->variables['FRONTEND_PATH'] . '/' . $match[1] .
                           '<?php
                                $includes[] = str_replace(\'//\', \'/\', eval(\'return \\\'\' . str_replace(\'\\\'\', \'\\\\\\\'\', ob_get_clean()) .\'\\\';\'));

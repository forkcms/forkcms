...
class FrontendBlogDetail extends FrontendBaseBlock
{
    ...
    private function loadForm()
    {
        ...
        $email   = (CommonCookie::exists('comment_email') && SpoonFilter::isEmail(
                                                                        CommonCookie::get('comment_email')
            )) ? CommonCookie::get('comment_email') : null;
        $website = (CommonCookie::exists('comment_website') && SpoonFilter::isURL(
                                                                          CommonCookie::get('comment_website')
            )) ? CommonCookie::get('comment_website') : 'http://';
    ...
    private function parse()
    {
        ...
        if (isset($this->record['meta_data']['seo_index'])) {
            $this->header->addMetaData(
                         array(
                              'name'    => 'robots',
                              'content' => $this->record['meta_data']['seo_index']
                         )
            );
        }
        if (isset($this->record['meta_data']['seo_follow'])) {
            $this->header->addMetaData(
                         array(
                              'name'    => 'robots',
                              'content' => $this->record['meta_data']['seo_follow']
                         )
            );
        }
            ...
            if ($this->frm->isCorrect()) {
                ...
                if (strpos($redirectLink, '?') === false) {
                    if ($comment['status'] == 'moderation') {
                        $redirectLink .= '?comment=moderation#' . FL::act(
                                                                    'Comment'
                            );
                    }

...
class BackendJavascript
{
    ...
    private function setFile($value)
    {
        ...
        if (substr_count($this->filename, '../') > 0) {
            ...
            if (SPOON_DEBUG) {
                throw new BackendException('Invalid file.');
        ...
        if ($this->module == 'core') {
            ...
            if (substr($path, 0, strlen(realpath(BACKEND_CORE_PATH . '/js/'))) != realpath(
                    BACKEND_CORE_PATH . '/js/'
                )
            ) {
                $valid = false;
            }
            ...
            if (substr(
                    $path,
                    0,
                    strlen(realpath(BACKEND_MODULES_PATH . '/' . $this->getModule() . '/js/'))
                ) != realpath(BACKEND_MODULES_PATH . '/' . $this->getModule() . '/js/')
            ) {
                $valid = false;
            }
        }

...
class BackendModel extends BaseModel
{
    ...
    public static function checkSettings()
    {
        ...
        if (BackendAuthentication::isAllowedAction('index', 'settings')) {
            ...
            if (self::getModuleSetting('core', 'fork_api_private_key') == '' || self::getModuleSetting(
                                                                                    'core',
                                                                                        'fork_api_public_key'
                                                                                ) == ''
            ) {
    ...
    public static function getModuleSettings($module = null)
    {
        ...
        if (empty(self::$moduleSettings)) {
            ...
            foreach ($moduleSettings as $setting) {
                ...
                // validate
                if ($value === false && serialize(
                                            false
                                        ) != $setting['value']
                ) {
                    throw new BackendException('The modulesetting (' . $setting['module'] . ': ' . $setting['name'] . ') wasn\'t saved properly.');
                }
    ...
    public static function getUTCTimestamp(SpoonFormDate $date, SpoonFormTime $time = null)
    {
        ...
        if (!$date->isValid() || ($time !== null && !$time->isValid())
            ...
            throw new BackendException('You need to provide two objects that actually contain valid data.');
        }
    ...
    {
        $siteTitle     = self::getModuleSetting(
                             'core',
        ...
        if ($pingServices === null || $pingServices['date'] < strtotime('-1 month')) {
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'Operation timed out') === false && strpos(
                                                                                     $e->getMessage(),
                                                                                     'Invalid headers'
                                                                                 ) === false
                ) {

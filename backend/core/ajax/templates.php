...
class BackendCoreAjaxTemplates extends BackendBaseAJAXAction
{
	...
	public function execute()
	{
		...
		if(BackendModel::getContainer()->get('filesystem')->exists($themePath)) $files[] = $themePath;
	...
	private function processFile($file)
	{
		...
		if(!BackendModel::getContainer()->get('filesystem')->exists($file)) return array();
		...
		foreach($json as $template)
		{
			...
			{
				if(BackendModel::getContainer()->get('filesystem')->exists(PATH_WWW . $template['file']))
				{

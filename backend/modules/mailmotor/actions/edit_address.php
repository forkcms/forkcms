...
class BackendMailmotorEditAddress extends BackendBaseActionEdit
{
	...
	private function getData()
	{
		...
		// no item found, throw an exceptions, because somebody is fucking with our URL
		if(empty($this->record)) return $this->redirect(BackendModel::createURLForAction('addresses') . '&error=non-existing');
	...
	private function loadForm()
	{
		...
		if(empty($checkboxGroups)) return $this->redirect(BackendModel::createURLForAction('addresses') . '&error=no-groups');
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				return $this->redirect(BackendModel::createURLForAction('addresses') . (!empty($this->subscriptions) ? '&group_id=' . $ddmGroups->getValue() : '') . '&report=edited&var=' . urlencode($item['email']) . '&highlight=email-' . $item['email']);
			}
		}
	}
}

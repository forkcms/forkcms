...
class BackendMailmotorEditCampaign extends BackendBaseActionEdit
{
	...
	private function getData()
	{
		...
		// no item found, throw an exceptions, because somebody is fucking with our URL
		if(empty($this->record)) return $this->redirect(BackendModel::createURLForAction('campaigns') . '&error=non-existing');
	}
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				return $this->redirect(BackendModel::createURLForAction('campaigns') . '&report=edited&var=' . urlencode($item['name']) . '&highlight=id-' . $item['id']);
			}
		}
	}
}

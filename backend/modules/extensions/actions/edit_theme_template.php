...
class BackendExtensionsEditThemeTemplate extends BackendBaseActionEdit
{
	...
	private function loadData()
	{
		...
		// validate id
		if($this->id === null || !BackendExtensionsModel::existsTemplate($this->id)) return $this->redirect(BackendModel::createURLForAction('theme_templates') . '&error=non-existing');
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				return $this->redirect(BackendModel::createURLForAction('theme_templates') . '&theme=' . $item['theme'] . '&report=edited-template&var=' . urlencode($item['label']) . '&highlight=row-' . $item['id']);
			}
		}
	}
}

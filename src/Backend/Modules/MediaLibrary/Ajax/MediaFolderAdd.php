...
class MediaFolderAdd extends BackendBaseAJAXAction
{
    public function execute(): void
    {
        ...
        $this->output(
            ...
            vsprintf(
                BackendLanguage::msg('FolderIsAdded'),
                [
    ...
    protected function getFolderName(MediaFolder $parent = null): string
    {
        ...
        if ($name === null) {
            throw new AjaxExitException(BackendLanguage::err('NameIsRequired'));
        }

        // Folder name already exists
        if ($this->get('media_library.repository.folder')->existsByName($name, $parent)) {
            throw new AjaxExitException(BackendLanguage::err('MediaFolderExists'));
        }

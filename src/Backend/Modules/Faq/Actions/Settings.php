...
class Settings extends BackendBaseActionEdit
{
    ...
    private function loadForm(): void
    {
        ...
        $this->form->addDropdown(
            ...
            $this->get('forkcms.settings')->get($this->url->getModule(), 'overview_num_items_per_category', 10)
        );
        $this->form->addDropdown(
            'most_read_number_of_items',
            array_combine(range(1, 10), range(1, 10)),
            $this->get('forkcms.settings')->get($this->url->getModule(), 'most_read_num_items', 10)
        );
        $this->form->addDropdown(
            'related_number_of_items',
            array_combine(range(1, 10), range(1, 10)),
            $this->get('forkcms.settings')->get($this->url->getModule(), 'related_num_items', 3)
        );
        $this->form->addCheckbox(
            'allow_multiple_categories',
            $this->get('forkcms.settings')->get($this->url->getModule(), 'allow_multiple_categories', false)
        );
        $this->form->addCheckbox(
            'spamfilter',
            $this->get('forkcms.settings')->get($this->url->getModule(), 'spamfilter', false)
        );
        $this->form->addCheckbox(
            'allow_feedback',
            $this->get('forkcms.settings')->get($this->url->getModule(), 'allow_feedback', false)
        );
        $this->form->addCheckbox(
            'allow_own_question',
            $this->get('forkcms.settings')->get($this->url->getModule(), 'allow_own_question', false)
        );
        $this->form->addCheckbox(
            'send_email_on_new_feedback',
            $this->get('forkcms.settings')->get($this->url->getModule(), 'send_email_on_new_feedback', false)
        );

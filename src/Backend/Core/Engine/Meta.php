...
class Meta
{
    ...
    public function __construct(Form $form, $metaId = null, $baseFieldName = 'title', $custom = false)
    {
        ...
        if(!BackendModel::getContainer()->has('url')) throw new Exception('URL should be available in the reference.');
    ...
    public function generateURL($URL)
    {
        ...
        if(!is_callable(array($this->callback['class'], $this->callback['method']))) throw new Exception('The callback-method doesn\'t exist.');
    ...
    protected function loadMeta($id)
    {
        ...
        if(empty($this->data)) throw new Exception('Meta-record doesn\'t exist.');
    ...
    public function save($update = false)
    {
        ...
        if($update) {
            // validate
            if($this->id === null) throw new Exception('No metaID specified.');
    ...
    public function validate()
    {
        ...
        if($this->frm->getField('url_overwrite')->isChecked()) {
            ...
            if($URL != $generatedUrl) $this->frm->getField('url')->addError(Language::err('URLAlreadyExists'));
        }

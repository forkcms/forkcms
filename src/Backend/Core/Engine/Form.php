...
class FormImage extends \SpoonFormImage
{
    ...
    public function parse(\SpoonTemplate $template = null)
    {
        ...
        if (strtoupper(substr($uploadMaxFilesize, -1, 1)) == 'K') $uploadMaxFilesize = substr(
                                                                                           $uploadMaxFilesize,
                                                                                           0,
                                                                                           -1
                                                                                       ) . 'kB';
        ...
        $output .= $this->getAttributesHTML(
                       array('[id]' => $this->attributes['id'], '[name]' => $this->attributes['name'])
                   ) . ' />';

        // add help txt if needed
        if (!$this->hideHelpTxt) {
            $output .= '<span class="helpTxt">' . sprintf(
...
class FormFile extends \SpoonFormFile
{
    ...
    public function parse(\SpoonTemplate $template = null)
    {
        ...
        // add help txt if needed
        if (!$this->hideHelpTxt) {
            if (isset($this->attributes['extension'])) $output .= '<span class="helpTxt">' . sprintf(
                    Language::getMessage('HelpFileFieldWithMaxFileSize', 'core'),
                    $this->attributes['extension'],
                    $uploadMaxFilesize
                ) . '</span>';
            else $output .= '<span class="helpTxt">' . sprintf(

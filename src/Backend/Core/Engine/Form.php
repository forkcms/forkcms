...
class Form extends \SpoonForm
{
     ...
     * @param string $action The action (URL) whereto the form will be submitted, if not provided it will be autogenerated.
    ...
    public function addButton($name, $value, $type = 'submit', $class = null)
    {
        ...
        // do a check
        if($type == 'submit' && $name == 'submit') throw new Exception('You can\'t add buttons with the name submit. JS freaks out when we replace the buttons with a link and use that link to submit the form.');
    ...
    public function addDate($name, $value = null, $type = null, $date = null, $date2 = null, $class = null, $classError = null)
    {
        ...
        // validate
        if($type == 'from' && ($date == 0 || $date == null)) throw new Exception('A datefield with type "from" should have a valid date-parameter.');
        if($type == 'till' && ($date == 0 || $date == null)) throw new Exception('A datefield with type "till" should have a valid date-parameter.');
        if($type == 'range' && ($date == 0 || $date2 == 0 || $date == null || $date2 == null)) throw new Exception('A datefield with type "range" should have 2 valid date-parameters.');
...
class FormFile extends \SpoonFormFile
{
    ...
    public function parse(\SpoonTemplate $template = null)
    {
        ...
        // add help txt if needed
        if(!$this->hideHelpTxt) {
            if(isset($this->attributes['extension'])) $output .= '<span class="helpTxt">' . sprintf(Language::getMessage('HelpFileFieldWithMaxFileSize', 'core'), $this->attributes['extension'], $uploadMaxFilesize) . '</span>';
            else $output .= '<span class="helpTxt">' . sprintf(Language::getMessage('HelpMaxFileSize'), $uploadMaxFilesize) . '</span>';
        }

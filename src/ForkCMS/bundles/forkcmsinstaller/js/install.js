...
var jsInstall = {
  init: function () {
    jsInstall.steps.init()
    jsInstall.controls.init()
    jsInstall.forms.init()
    jsInstall.layout.init()
  }
}

jsInstall.steps = {
  init: function () {
    jsInstall.steps.step2.init()
    jsInstall.steps.step3.init()
    jsInstall.steps.step4.init()
    jsInstall.steps.step6.init()
  }
}

jsInstall.steps.step2 = {
  init: function () {
    jsInstall.steps.step2.toggleLanguageType()
    jsInstall.steps.step2.handleLanguageChanges()
    jsInstall.steps.step2.setInterfaceDefaultLanguage()
  },
...
jsInstall.steps.step3 = {
  init: function () {
    jsInstall.steps.step3.toggleDebugEmail()
  },
...
jsInstall.controls = {
  ...
  init: function () {
    jsInstall.controls.bindPasswordStrengthMeter()
  },
  ...
  bindPasswordStrengthMeter: function () {
    ...
    if ($passwordStrength.length > 0) {
      $passwordStrength.each(function () {
        ...
        var strength = jsInstall.controls.checkPassword($('#' + id).val())
...
jsInstall.forms = {
  // init, something like a constructor
  init: function () {
    jsInstall.forms.focusFirstField()
    jsInstall.forms.submitWithLinks()
  },
...
jsInstall.layout = {
  ...
  init: function () {
    ...
    jsInstall.layout.showBrowserWarning()
    jsInstall.layout.dataGrid()

    if ($('.dataFilter').length > 0) { jsInstall.layout.dataFilter() }

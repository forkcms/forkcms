...
$.Autocompleter = function(input, options) {
	...
	function lastWord(value) {
		...
		var cursorAt = $(input).selection().start;
		if(cursorAt == value.length) { 	words = trimWords(value)
		} else { words = trimWords(value.replace(value.substring(cursorAt), "")); }
	...
	function request(term, success, failure) {
		...
		if(data && data.length) { success(term, data);
			...
			$.ajax({ mode: "abort", port: "autocomplete" + input.name, dataType: options.dataType, url: options.url,

...
jsBackend.pages.extras = { init: function()
{
	// bind events
...
load: function()
{
	...
	{
		jsBackend.pages.extras.changeExtra($(this).val(), $(this).attr('id').replace('blockExtraId', ''));
	})
	// initialize the modal for choosing an extra
	if($('#chooseExtra').length > 0)
	{
		$('#chooseExtra').dialog( { autoOpen: false, draggable: false, resizable: false, modal: true, width: 500, buttons: { '{$lblOK|ucfirst}': function()
		{
	...
	{
		$('#chooseTemplate').dialog( { autoOpen: false, draggable: false, resizable: false, modal: true, width: 500, buttons: { '{$lblOK|ucfirst}': function()
		{
...
showExtraDialog: function(evt)
{
	...
	{
		var id = $(this).val();
		if(id != '' && typeof extrasById[id] != 'undefined' && extrasById[id].type == 'block')
			hasModules = true;
	});
	// blocks linked?
	if(hasModules)
...
populateExtraModules: function()
{
	...
	if(selectedType == 'widget' || selectedType == 'block')
	{
		...
		for( var i in extrasData)
		{
			...
			if(typeof extrasData[i]['items'][selectedType] != 'undefined')
				$('#extraModule').append('<option value="' + extrasData[i].value + '">' + extrasData[i].name + '</option>');
		}
		// show
...
jsBackend.pages.manageTemplates = { init: function()
{
...
jsBackend.pages.template = { init: function()
{
	// bind events
...
// end
eoo: true }
jsBackend.pages.tree = { init: function()
{
	if($('#tree div').length == 0)
		return false;
	// add "treeHidden"-class on leafs that are hidden, only for browsers that
	...
	if(typeof pageID != 'undefined')
	{
		...
		for( var i = 0; i < parents.length; i++)
			openedIds.push($(parents[i]).attr('id'));
	}
	// add home if needed
	if(!utils.array.inArray('page-1', openedIds))
		openedIds.push('page-1');
	var options = { ui: { theme_name: 'fork' }, opened: openedIds, rules: { multiple: false, multitree: 'all', drag_copy: false }, lang: { loading: '{$lblLoading|ucfirst}' }, callback: { beforemove: jsBackend.pages.tree.beforeMove, onselect: jsBackend.pages.tree.onSelect, onmove: jsBackend.pages.tree.onMove }, types: { 'default': { renameable: false, deletable: false, creatable: false, icon: { image: '/backend/modules/pages/js/jstree/themes/fork/icons.gif' } }, 'page': { icon: { position: '0 -80px' } }, 'folder': { icon: { position: false } }, 'hidden': { icon: { position: false } }, 'home': { draggable: false, icon: { position: '0 -112px' } }, 'pages': { icon: { position: false } }, 'error': { draggable: false, max_children: 0, icon: { position: '0 -160px' } }, 'sitemap': { max_children: 0, icon: { position: '0 -176px' } } }, plugins: { cookie: { prefix: 'jstree_', types: { selected: false }, options: { path: '/' } } } };
	// create tree
	$('#tree div').tree(options);
	// layout fix for the tree
	$('.tree li.open').each(function()
	{
		// if the so-called open-element doesn't have any childs we should
		// replace the open-class.
...
beforeMove: function(node, refNode, type, tree)
{
	...
	url: '/backend/ajax.php?module=pages&action=get_info&language={$LANGUAGE}', data: 'id=' + currentPageID, error: function(XMLHttpRequest, textStatus, errorThrown)
	{
		if(jsBackend.debug)
			alert(textStatus);
		result = false;
...
}, onMove: function(node, refNode, type, tree, rollback)
{
	...
	// make the call
	$.ajax( { url: '/backend/ajax.php?module=pages&action=move&language={$LANGUAGE}', data: 'id=' + currentPageID + '&dropped_on=' + droppedOnPageID + '&type=' + type, success: function(json, textStatus)
	{
		if(json.code != 200)
		{
			if(jsBackend.debug)

<div id="toc" class="col-3">
	<div class="content">
		<h4 class="secondaryContent">Table of Contents</h4>
		<ol>
			<li>
				<a href="index.php">Introduction</a>
				<ol>
					<li><a href="index.php#whatis">What is Fork CMS?</a></li>
					<li><a href="index.php#download">Download</a></li>
					<li><a href="index.php#why">Why?</a></li>
					<li><a href="index.php#sysreq">System requirements</a></li>
					<li><a href="index.php#installation">Installation</a></li>
					<li><a href="index.php#thanks">Thanks!</a></li>
				</ol>
			</li>
			<li>
				<a href="themes.php">Themes</a>
				<ol>
					<li><a href="themes.php#howitworks">How themes work</a></li>
					<li><a href="themes.php#structure">Theme structure</a></li>
					<li><a href="themes.php#overrides"><s>Overrides</s></a></li>
					<li><a href="themes.php#templateModifiers">Template modifiers</a></li>
					<li><a href="themes.php#constants">Available constants</a></li>
				</ol>
			</li>
			<li>
				<a href="modules.php">Modules</a>
				<ol>
					<li><a href="modules.php#intro">Introduction</a></li>
					<li><a href="modules.php#translations">Translations</a></li>
					<li><a href="modules.php#pages"><s>Pages</s></a></li>
					<li><a href="modules.php#contentBlocks"><s>Content blocks</s></a></li>
					<li><a href="modules.php#blog"><s>Blog</s></a></li>
				</ol>
			</li>
			<li>
				<a href="module_development.php">Module development</a>
				<ol>
					<li><a href="module_development.php#backendConstants">Available constants</a></li>
					<li><a href="module_development.php#backendModifiers">Template modifiers</a></li>
				</ol>
			</li>
			<li>
				<a href="sysreq.php">Detailed system requirements</a>
			</li>
			<li>
				<a href="extra.php">Unsorted documentation</a>
			</li>
		</ol>
	</div>
</div>
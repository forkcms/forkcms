...
class SpoonThumbnail
{
	...
	public function parse($headers = true)
	{
		...
		if($imageProperties === false) throw new SpoonImageException('The sourcefile "'. $this->filename .'" could not be found.');
	...
	public function parseToFile($filename, $quality = 100, $chmod = 0777)
	{
		...
		{
			// strict?
			if($this->strict) throw new SpoonImageException('The sourcefile "'. $this->filename .'" could not be found.');
			return false;
		}

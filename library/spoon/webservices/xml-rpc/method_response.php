 ...
 * @since			1.0.0
 */
class SpoonXMLRPCMethodResponse
{
	/**
	 * Allowed types
	 *
	 * @var	array
	 */
	private $aAllowedTypes = array('', 'array', 'base64', 'bool', 'dateTime.iso8601', 'double', 'int', 'string', 'struct');


	/**
	 * is this a error-response
	 *
	 * @var	bool
	 */
	private $error = false;
	...
	private function buildArray($array, $xml = '')
	{
		...
		{
			$xml .= $this->buildValue($element['type'], $element['value']);
		}
	...
	private function buildValue($type, $value)
	{
		switch ($type)
		{
			...
			case 'struct':
				...
				{
					$struct .= str_replace("\n", '', '<member>'. "\n" .'<name>'. $key .'</name>'. self::buildValue(gettype($value), $value) .'</member>') . "\n";
				}
				...
				$xml .= '<value>'."\n";
					$xml .= '<struct>'."\n";
						$xml .= '<member>'."\n";
							...
							$xml .= '<value><int>'. $this->errorCode .'</int></value>'."\n";
						$xml .= '</member>'."\n";
						$xml .= '<member>'."\n";
							$xml .= '<name>faultString</name>'."\n";
							$xml .= '<value><string>'. $this->errorMessage .'</string></value>'."\n";
						$xml .= '</member>'."\n";
					$xml .= '</struct>'."\n";
				$xml .= '</value>'."\n";
			$xml .= '</fault>'."\n";
	 ...
	 */
	public function setFault($code, $message)
	{
		$this->error = true;

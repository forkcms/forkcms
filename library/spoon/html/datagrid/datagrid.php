...
class SpoonDataGrid
{
	 ...
	 * 
	 * @return	void
	 * @param	SpoonDataGridSource
	 * @param	string[optional] $template
	 */
	public function __construct($source, $template = null)
	{
		// set source
	...
	private function parse()
	{
		...
		if($this->source->getNumResults() > 0)
		{
			...
			{
				// load template
				$this->tpl = new SpoonTemplate($this->getTemplatePath());
				
				// disable headers
				$this->tpl->disableHeaders();
				...
				// parse to buffer
				$this->content = $this->tpl->getContent();
	...
	private function parseBody(array $records)
	{
		...
		// iterations exist
		$iterationRows = ($this->tpl->iterationExists('iRows')) ? true : false;
		$iterationColumns = ($this->tpl->iterationExists('iColumns')) ? true : false;
		
		// set iteration
		if($iterationRows) $this->tpl->setIteration('iRows');
		
		// loop records
		foreach($records as $i => &$record)
		{
			// reset row
			$aRow = array('attributes' => '', 'oddAttributes' => '', 'evenAttributes' => '');
			...
			// parse vars
			if($iterationRows) $this->tpl->assignIterationArray($aRow, 'row');
			...
			foreach($aSequence as $name)
			{
				...
				else
				{
					...
					if($column->getUrl() !== null)
					{
						...
						// confirm
						if($column->getConfirm() && $column->getConfirmMessage() !== null) $columnValue .= $columnValue .= ' onclick="return confirm(\''. str_replace($record['labels'], $record['values'], $column->getConfirmMessage()) .'\');"';
				...
				// visible & iteration
				if(!$column->getHidden() && $iterationColumns)
				{
					// set iteration
					$this->tpl->setIteration('iColumns', 'iRows');
					
					// parse vars
					$this->tpl->assignIterationArray(array('attributes' => $columnAttributes, 'value' => $columnValue), 'column');
					
					// refill iteration
					$this->tpl->refillIteration();
				}
				
				// add to special array
				$aColumns[$column->getName()] = $columnValue;
			}
			
			// parse iteration
			if($this->tpl->getIteration() == 'iColumns') $this->tpl->parseIteration();
			
			// refill
			if($iterationRows)
			{
				// parse row 
				$this->tpl->assignIterationArray($aColumns, 'column.');
		...
		// parse iteration
		if($iterationRows) $this->tpl->parseIteration();
	}
	...
	private function parseCaption()
	{
		...
		{
			$this->tpl->assignOption('oCaption');
			$this->tpl->assign('caption', $this->caption);
		}
	}
	...
	{
		// attributes
		$this->tpl->assign('footer.attributes', $this->getHtmlAttributes($this->attributes['footer']));
	...
	{
		// attributes
		$this->tpl->assign('header.attributes', $this->getHtmlAttributes($this->attributes['header']));
		
		// sequence
		$aSequence = $this->getColumnsSequence();
		
		// iteration exists
		$iteration = ($this->tpl->iterationExists('iHeader')) ? true : false;
		...
		foreach($aSequence as $name)
		{
			...
			// visible & iteration
			if(!$column->getHidden() && $iteration)
			{
				// set iteration
				$this->tpl->setIteration('iHeader');
				
				// sorting globally enabled AND for this column
				if($sorting && in_array($name, $aSortingColumns))
				{
					// show option
					$this->tpl->assignIterationOption('oSorting');
					
					// sorted on this column?
					if($this->getOrder() == $name)
					{
						// show option
						$this->tpl->assignIterationOption('oSorted');
						
						// asc
						if($this->getSort() == 'asc') $this->tpl->assignIterationOption('oSortedAsc');
						
						// desc
						else $this->tpl->assignIterationOption('oSortedDesc');
					}
					 ...
					 * sorted on this column
					 */
					elseif(in_array($name, $aSortingColumns)) $this->tpl->assignIterationOption('oNotSorted');
					...
					// build actual urls
					$aUrls['url'] = $this->buildURL($this->getOffset(), $name, $sortingMethod);
					$aUrls['urlAsc'] = $this->buildURL($this->getOffset(), $name, 'asc');
					$aUrls['urlDesc'] = $this->buildURL($this->getOffset(), $name, 'desc');
					
					// parse url's
					$this->tpl->assignIterationArray($aUrls, 'sorting.');
					...
					// asc & desc icons
					$aIcons['icon'] = $sortingIcon;
					$aIcons['iconAsc'] = ($this->getSort() == 'asc') ? $this->sortingIcons['ascSelected'] : $this->sortingIcons['asc']; 
					$aIcons['iconDesc'] = ($this->getSort() == 'desc') ? $this->sortingIcons['descSelected'] : $this->sortingIcons['desc'];
					
					// parse icons
					$this->tpl->assignIterationArray($aIcons, 'sorting.');
					...
					$aLabels['label'] = $sortingLabel;
					$aLabels['labelAsc'] = $this->sortingLabels['asc'];
					$aLabels['labelDesc'] = $this->sortingLabels['desc'];
					
					// parse labels
					$this->tpl->assignIterationArray($aLabels, 'sorting.');
				}
				
				// no sorting enabled for this column
				else $this->tpl->assignIterationOption('oNoSorting');
				
				// parse vars
				$this->tpl->assignIteration('label', $column->getLabel());
			...
			// add to header array
			$aHeader[$column->getName()] = $column->getLabel();
		}
		
		// parse array
		if(isset($aHeader)) $this->tpl->assignArray($aHeader, 'header');
		
		// parse iteration
		if($this->tpl->getIteration() == 'iHeader') $this->tpl->parseIteration();
	}
	...
	{
		// enabled?
		if($this->paging) $this->tpl->assignOption('oPaging');
		
		// offset, order & sort
		$this->tpl->assign(array('iOffset', 'order', 'sort'), array($this->getOffset(), $this->getOrder(), $this->getSort()));

		// number of results
		$this->tpl->assign('iResults', $this->source->getNumResults());
		
		// number of pages
		// @todo: what happens if you set your paging limit to zero ?
		...
		// parse paging
		$content = SpoonDataGridPaging::getContent($this->url, $this->getOffset(), $this->getOrder(), $this->getSort(), $this->source->getNumResults(), $this->pagingLimit);
	...
	private function parseSummary()
	{
		...
		{
			$this->tpl->assignOption('oSummary');
			$this->tpl->assign('summary', $this->caption);
		}
	}
	...
	public function setSortingColumns(array $columns, $default = null)
	{
		...
		if($this->source->getNumResults() > 0)
		{
			...
			// default column set
			if($default !== null) $defaultColumn = (string) $default;
	...
	{
		// valid source
		if(is_object($source) && is_subclass_of($source, 'SpoonDataGridSource')) $this->source = $source;
		
		// invalid
		else throw new SpoonDataGridException('Only classes that extend SpoonDataGridSource are allowed sources.');
	}
	...
	// javasript = string, wordt aan de <a {$javascript} gehangen!
	public function setColumnConfirm($column, $message, $javascript = null)
	{

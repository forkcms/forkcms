...
class SpoonDatabaseMysqli implements iSpoonDatabaseObject
{
	...
	private function prepareQuery($query, $parameters)
	{
		...
		if(!empty($parameters))
		{
			...
			// validate
			if($countQuestionmarks != $countParameters) throw new SpoonDatabaseException('The amount of questionmarks need to be the same as number of parameters.', 0, $this->password);
			...
			foreach($parameters as $parameter)
			{
				...
				{
					// booleans should be send as an integer-value
					case 'boolean':
						$aReplace[] = ($parameter) ? 1 : 0;
					break;
